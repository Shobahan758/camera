...
@section('content')
    ...
    @endphp
    <div class="page-heading"><div><h1>অর্ডার এডিট</h1><p>#{{ $order->id }} নম্বর অর্ডারের তথ্য পরিবর্তন করুন।</p></div><a href="{{ $backUrl }}" style="color:#3730a3;text-decoration:none"><i class="fa-solid fa-arrow-left"></i> তালিকায় ফিরুন</a></div>
    <section class="panel" style="max-width:760px">
        <form method="POST" action="{{ route('admin.orders.update', $order) }}">
            ...
            @foreach([['name','নাম','text'],['phone','ফোন নম্বর','tel'],['email','ইমেইল','email']] as [$name,$label,$type])
                ...
                <input id="{{ $name }}" name="{{ $name }}" type="{{ $type }}" value="{{ old($name, $order->$name) }}" @required($name !== 'email') style="width:100%;padding:11px 12px;border:1px solid #dbe3ee;border-radius:9px;font:inherit">
                @error($name)<div style="color:#dc3545;font-size:13px">{{ $message }}</div>@enderror<div style="height:12px"></div>
            @endforeach
            <label for="address" style="display:block;margin-bottom:6px;font-weight:600">ঠিকানা *</label><textarea id="address" name="address" rows="4" required style="width:100%;padding:11px 12px;border:1px solid #dbe3ee;border-radius:9px;font:inherit;resize:vertical">{{ old('address', $order->address) }}</textarea>@error('address')<div style="color:#dc3545;font-size:13px">{{ $message }}</div>@enderror<div style="height:12px"></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px">
                <div><label for="quantity" style="display:block;margin-bottom:6px;font-weight:600">পরিমাণ *</label><input id="quantity" name="quantity" type="number" min="1" value="{{ old('quantity', $order->quantity) }}" required style="width:100%;padding:11px 12px;border:1px solid #dbe3ee;border-radius:9px;font:inherit">@error('quantity')<div style="color:#dc3545;font-size:13px">{{ $message }}</div>@enderror</div>
                <div><label for="unit_price" style="display:block;margin-bottom:6px;font-weight:600">একক মূল্য *</label><input id="unit_price" name="unit_price" type="number" min="0" value="{{ old('unit_price', $order->unit_price) }}" required style="width:100%;padding:11px 12px;border:1px solid #dbe3ee;border-radius:9px;font:inherit">@error('unit_price')<div style="color:#dc3545;font-size:13px">{{ $message }}</div>@enderror</div>
            </div>
